...
{
	ILI9341_Hardware_Init();//初始化ILI9341的IO引脚
	ILI9341_FSMC_Hardware_Init();//LCD  FSMC 模式配置
	ILI9341_BackLed_Control(ENABLE);//点亮LCD背光灯
	ILI9341_Rst();                  //ILI9341 软件复位
	ILI9341_REG_Init();             //初始化ILI9341寄存器
	ILI9341_GramScan(6);//设置默认扫描方向，其中 6 模式为大部分液晶例程的默认显示方向
	LCD_SetColors(RED,BLACK);       //黑底红字
	ILI9341_Clear(0,0,320,320);
}
...
static void ILI9341_Hardware_Init ( void )
{
  ...
  * @retval 无
  */
static void ILI9341_FSMC_Hardware_Init( void )
{
 ...
 * @retval 无
 */
static void ILI9341_REG_Init(void)
{
