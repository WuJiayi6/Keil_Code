...
void DMA_Hardware_Init(void)
{
	...
	DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralSRC;//方向:外设到存储器（这里的外设是内部的FLASH）
...
//	DMA_InitStructure.DMA_Mode = DMA_Mode_Normal ;// DMA模式，一次
	...
	DMA_Init(DMA1_Channel6, &DMA_InitStructure);// 配置DMA通道6
	DMA_ClearFlag(DMA1_FLAG_TC6);//清除DMA数据流传输完成标志位
