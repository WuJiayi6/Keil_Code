...
volatile uint16_t ADC_Value[CHANEL_NUM]={0,0,0,0};
...
void ADC1_GPIO_Init(void)
{
	...
	RCC_APB2PeriphClockCmd (RCC_APB2Periph_GPIOC, ENABLE );// 打开 ADC IO端口时钟
    // 配置 ADC IO 引脚模式
	GPIO_InitStructure.GPIO_Pin = 	GPIO_Pin_0|GPIO_Pin_1|GPIO_Pin_2|GPIO_Pin_3|GPIO_Pin_4|GPIO_Pin_5;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AIN;
	GPIO_Init(GPIOC, &GPIO_InitStructure);// 初始化 ADC IO				
}
...
void Init_All_Periph(void)
{
    ...
    ILI9341_DispStringLine_EN_CH(LINE(1),"-----显示-----");//LCD显示字符串
...
void LCD_Show(void)
{
    sprintf(show0," PC0:%5d VOL:%.2f V",ADC_Value[0],(float)(ADC_Value[0])*3.3/4095);
    sprintf(show1," PC1:%5d VOL:%.2f V",ADC_Value[1],(float)(ADC_Value[1])*3.3/4095);
    sprintf(show2," PC2:%5d VOL:%.2f V",ADC_Value[2],(float)(ADC_Value[2])*3.3/4095);
    sprintf(show3," PC3:%5d VOL:%.2f V",ADC_Value[3],(float)(ADC_Value[3])*3.3/4095);
    sprintf(show4," PC4:%5d VOL:%.2f V",ADC_Value[4],(float)(ADC_Value[4])*3.3/4095);
    sprintf(show5," PC5:%5d VOL:%.2f V",ADC_Value[5],(float)(ADC_Value[5])*3.3/4095);
    ...
    ILI9341_DispStringLine_EN(LINE(6),show3);//LCD显示字符串
    ILI9341_DispStringLine_EN(LINE(7),show4);//LCD显示字符串
    ILI9341_DispStringLine_EN(LINE(8),show5);//LCD显示字符串
    Delay_ms(50);
}
